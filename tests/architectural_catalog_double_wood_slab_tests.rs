...
use scene::overworld_blocks::double_wood_slab_material_id;
...
fn faces(e: &Env) -> core::face_textures::FaceTextures {
    e.library
        .get(double_wood_slab_material_id())
        .unwrap()
        .face_textures
        .unwrap()
}
...
#[test]
fn doublewoodslab_is_a_full_cube_block_type() {
    let geometry = block_geometry(BlockType::DoubleWoodSlab, Orientation::Up);
...
fn the_material_is_opaque_non_emissive_and_matches_its_profile() {
    let e = env();
    let m = e.library.get(double_wood_slab_material_id()).unwrap();
...
fn it_is_an_official_catalog_entry_with_a_finite_focus_point() {
    ...
    let entry = scene
        .entries()
        .iter()
        .find(|e| e.material_id == double_wood_slab_material_id())
        .expect("in the catalog");

    assert_eq!(entry.display_name, "Double wood slab");
    assert_eq!(entry.block_type, BlockType::DoubleWoodSlab);
    assert_eq!(entry.kind, SampleKind::PlainBlock);
    assert_eq!(entry.material_id, double_wood_slab_material_id());
...
fn there_is_exactly_one_sample_of_this_block() {
    let count = catalog_entries()
        .iter()
        .filter(|e| e.material_id == double_wood_slab_material_id())
        .count();
    assert_eq!(count, 1);
        ...
        for y in -1..5 {
            for z in -10..12 {
                if let Some(b) = scene.world().get(IVec3::new(x, y, z)) {
                    if b.material_id() == double_wood_slab_material_id() {
                        uses += 1;
                    }
                }
            }
...
#[test]
fn the_slab_reuses_the_wood_planks_texture_but_is_its_own_block() {
    let e = env();
    let slab = faces(&e);
    let planks = e
        .library
        .get(scene::overworld_blocks::wood_planks_material_id())
        .unwrap()
        .face_textures
        .unwrap();
    ...
    for face in FACES {
        assert_eq!(
            slab.texture_for_face(face),
            planks.texture_for_face(face),
            "{face:?}"
        );
    }
    assert_ne!(
        double_wood_slab_material_id(),
        scene::overworld_blocks::wood_planks_material_id()
    );
    assert_ne!(BlockType::DoubleWoodSlab, BlockType::WoodPlanks);
}

#[test]
fn the_slab_is_a_whole_cube_not_a_half_slab() {
    ...
    // A ray at 90% height still hits the front face, and the top face is at 1.0.
    let r = core::ray::Ray::new(
        core::math::Vec3::new(0.5, 0.9, 5.0),
        core::math::Vec3::new(0.0, 0.0, -1.0),
    );
    let hit = renderer::raytracer::nearest_voxel_hit(&world, &r, 50.0).expect("full height");
    assert!((hit.hit.point.z - 1.0).abs() < 1e-4);
    let down = core::ray::Ray::new(
        core::math::Vec3::new(0.5, 5.0, 0.5),
        core::math::Vec3::new(0.0, -1.0, 0.0),
    );
    let top = renderer::raytracer::nearest_voxel_hit(&world, &down, 50.0).unwrap();
    assert!(
        (top.hit.point.y - 1.0).abs() < 1e-4,
