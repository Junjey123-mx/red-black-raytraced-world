...
use scene::overworld_blocks::wood_planks_material_id;
...
#[test]
fn woodplanks_is_a_full_cube_block_type() {
    let geometry = block_geometry(BlockType::WoodPlanks, Orientation::Up);
...
fn the_material_is_opaque_non_emissive_and_matches_its_profile() {
    let e = env();
    let m = e.library.get(wood_planks_material_id()).unwrap();
...
fn it_is_an_official_catalog_entry_with_a_finite_focus_point() {
    ...
    let entry = scene
        .entries()
        .iter()
        .find(|e| e.material_id == wood_planks_material_id())
        .expect("in the catalog");

    assert_eq!(entry.display_name, "Wood planks");
    assert_eq!(entry.block_type, BlockType::WoodPlanks);
    assert_eq!(entry.kind, SampleKind::PlainBlock);
    assert_eq!(entry.material_id, wood_planks_material_id());
...
fn there_is_exactly_one_sample_of_this_block() {
    let count = catalog_entries()
        .iter()
        .filter(|e| e.material_id == wood_planks_material_id())
        .count();
    assert_eq!(count, 1);
        ...
        for y in -1..5 {
            for z in -10..12 {
                if let Some(b) = scene.world().get(IVec3::new(x, y, z)) {
                    if b.material_id() == wood_planks_material_id() {
                        uses += 1;
                    }
                }
            }
...
#[test]
fn planks_read_as_light_horizontal_boards_not_a_log() {
    let e = env();
    let f = faces(&e);
    ...
    // Boards run horizontally: rows are smoother than columns.
    let (mut dh, mut dv) = (0.0, 0.0);
    for y in 0..15 {
        for x in 0..15 {
            dh += (lum(x, y) - lum(x + 1, y)).abs();
            dv += (lum(x, y) - lum(x, y + 1)).abs();
        }
    }
    assert!(dh < dv, "horizontal planks: {dh} vs {dv}");
    ...
    let log = e
        .library
        .get(scene::overworld_blocks::log_material_id())
        .unwrap()
        .face_textures
        .unwrap();
    assert_ne!(
        f.texture_for_face(Face::PositiveY),
        log.texture_for_face(Face::PositiveY)
    );
    assert_ne!(
        f.texture_for_face(Face::PositiveY),
        log.texture_for_face(Face::PositiveZ)
    );
}

#[test]
fn planks_have_no_normal_map_and_are_semimatte() {
    let e = env();
    let m = e.library.get(wood_planks_material_id()).unwrap();
    assert!(m.normal_texture.is_none());
    assert!(m.specular <= 0.1 && m.reflectivity <= 0.02);
}
