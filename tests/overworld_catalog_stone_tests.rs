...
use scene::overworld_blocks::stone_block_material_id;
...
#[test]
fn stone_is_a_full_cube_block_type() {
    let geometry = block_geometry(BlockType::Stone, Orientation::Up);
...
fn the_material_is_opaque_non_emissive_and_matches_its_profile() {
    let e = env();
    let m = e.library.get(stone_block_material_id()).unwrap();

    assert_eq!(m.transparency, 0.0);
    assert_eq!(m.emission_strength, 0.0);
    assert!(m.emissive_texture.is_none());
    assert_eq!(m.refractive_index, 1.0);
    assert!((m.specular - 0.08).abs() < 1e-6, "specular {}", m.specular);
    assert!((m.shininess - 12.0).abs() < 1e-6);
...
fn it_is_an_official_catalog_entry_with_a_finite_focus_point() {
    ...
    let entry = scene
        .entries()
        .iter()
        .find(|e| e.material_id == stone_block_material_id())
        .expect("in the catalog");

    assert_eq!(entry.display_name, "Stone");
    assert_eq!(entry.block_type, BlockType::Stone);
    assert_eq!(entry.kind, SampleKind::PlainBlock);
    assert_eq!(entry.material_id, stone_block_material_id());
...
fn there_is_exactly_one_sample_of_this_block() {
    let count = catalog_entries()
        .iter()
        .filter(|e| e.material_id == stone_block_material_id())
        .count();
    assert_eq!(count, 1);
        ...
        for y in -1..5 {
            for z in -10..12 {
                if let Some(b) = scene.world().get(IVec3::new(x, y, z)) {
                    if b.material_id() == stone_block_material_id() {
                        uses += 1;
                    }
                }
            }
...
#[test]
fn stone_is_a_smooth_low_contrast_grey_unlike_cobblestone_or_deepslate() {
    let e = env();
    let stats = |id| {
        let f = e.library.get(id).unwrap().face_textures.unwrap();
        let t = e.manager.get(f.texture_for_face(Face::PositiveZ)).unwrap();
        let l: Vec<f32> = t.pixels().iter().map(|c| luminance(*c)).collect();
        let mean = l.iter().sum::<f32>() / l.len() as f32;
        let std = (l.iter().map(|x| (x - mean).powi(2)).sum::<f32>() / l.len() as f32).sqrt();
        (mean, std)
    };
    let (mean, std) = stats(stone_block_material_id());
    let (cobble_mean, cobble_std) = stats(scene::overworld_blocks::cobblestone_material_id());
    let (deep_mean, _) = stats(scene::overworld_blocks::deepslate_material_id());

    assert!(mean > 0.3 && mean < 0.6, "medium grey: {mean}");
    assert!(
        std < cobble_std,
        "less contrast than cobblestone: {std} vs {cobble_std}"
    );
    assert!(std < 0.09, "low contrast: {std}");
    assert!(
        mean > deep_mean * 1.2,
        "lighter than deepslate: {mean} vs {deep_mean}"
    );
    let _ = cobble_mean;
    ...
    for c in e
        .manager
        .get(f.texture_for_face(Face::PositiveY))
        .unwrap()
        .pixels()
    {
        assert!((c.r - c.b).abs() < 0.08, "{c:?}");
    }
}

#[test]
fn stone_has_no_normal_map_and_the_control_cubes_are_untouched() {
    let e = env();
    assert!(
        e.library
            .get(stone_block_material_id())
    ...
    // The diagnostic control and reflective cubes (also full stone-type cubes)
    // are still present and keep their own materials.
    let entries = catalog_entries();
    for (name, id) in [
        ("Control cube", scene::material_gallery::matte_material_id()),
        (
            "Reflective cube",
            scene::material_gallery::mirror_material_id(),
        ),
    ] {
        let entry = entries.iter().find(|e| e.display_name == name).expect(name);
        assert_eq!(entry.material_id, id);
        assert_ne!(entry.material_id, stone_block_material_id());
    }
}
