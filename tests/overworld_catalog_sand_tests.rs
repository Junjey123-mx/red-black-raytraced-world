...
#[test]
fn sand_is_a_full_cube_block_type() {
    let geometry = block_geometry(BlockType::Sand, Orientation::Up);
...
fn the_material_is_opaque_non_emissive_and_matches_its_profile() {
    let e = env();
    let m = e.library.get(sand_material_id()).unwrap();
...
fn it_is_an_official_catalog_entry_with_a_finite_focus_point() {
    ...
    let entry = scene
        .entries()
        .iter()
        .find(|e| e.block_type == BlockType::Sand)
        .expect("in the catalog");

    assert_eq!(entry.display_name, "Sand");
    assert_eq!(entry.kind, SampleKind::PlainBlock);
    assert_eq!(entry.material_id, sand_material_id());
...
fn there_is_exactly_one_sample_of_this_block() {
    let count = catalog_entries()
        .iter()
        .filter(|e| e.block_type == BlockType::Sand)
        .count();
    assert_eq!(count, 1);
        ...
        for y in -1..5 {
            for z in -10..12 {
                if let Some(b) = scene.world().get(IVec3::new(x, y, z)) {
                    if b.material_id() == sand_material_id() {
                        uses += 1;
                    }
                }
            }
...
#[test]
fn sand_is_light_warm_and_neither_dirt_nor_stone() {
    let e = env();
    let f = faces(&e);
    ...
    assert!(r > 0.6 && g > 0.55, "light: {r} {g} {b}");
    assert!(r >= g && g > b, "warm beige: {r} {g} {b}");

    let ids = f.texture_for_face(Face::PositiveY);
    for other in [
        scene::overworld_blocks::dirt_material_id(),
        scene::overworld_blocks::cobblestone_material_id(),
    ] {
        let o = e.library.get(other).unwrap().face_textures.unwrap();
        assert_ne!(ids, o.texture_for_face(Face::PositiveY));
    }
}

#[test]
fn sand_is_static_with_no_physics_dependency() {
    // The block is only data: a full cube placed in the world, and neither
    // the catalog nor the block modules mention gravity or falling.
    for path in ["src/scene/catalog.rs", "src/scene/overworld_blocks.rs"] {
        let source =
        ...
        let code: String = source
            ...
            .to_lowercase();
        assert!(
            !code.contains("gravity") && !code.contains("falling"),
            "{path}"
        );
    }
}
